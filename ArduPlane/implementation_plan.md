...
HDGALT adds two MAVLink messages to `ardupilotmega.xml`, which lives
in the `modules/mavlink` **submodule** (canonical:
`github.com/ArduPilot/mavlink`). Submodule changes can't be pushed
...
- `github.com/lvdlvd/mavlink` — fork of **ArduPilot/mavlink** (not
  mavlink/mavlink). Holds the `ardupilotmega.xml` change on branch
  `hdgaltmode`.
...
- Editing messages: `cd modules/mavlink`, edit
  `message_definitions/v1.0/ardupilotmega.xml`, commit **inside the
...
1. **mavlink PR:** `lvdlvd/mavlink:hdgaltmode` →
   `ArduPilot/mavlink:master`, containing only the
   `ardupilotmega.xml` change (messages, enums,
   `PLANE_MODE_HDGALT`). Get it reviewed/merged there.
