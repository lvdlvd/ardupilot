...
`modules/mavlink@hdgaltmode` commit `cc3f34ca` (move to
`hdgalt_dev.xml`); earlier commit `2f5675fa`, parent `7412a4a66a`.
`HDGALT_COMMAND` = id 52100, `HDGALT_STATE` = id 52101,
`PLANE_MODE_HDGALT` = 27, supporting enums added; `./waf plane`
builds and generates the headers.

The messages/enums now live in their own leaf dialect file
`modules/mavlink/message_definitions/v1.0/hdgalt_dev.xml`, pulled into
`ardupilotmega.xml` by a single `<include>` line. IDs were moved from
the original 11061/11062 into the experimental 52100/52101 range
(clear of the AIRLINK pair at 52000/52001) at the same time. To
revert the whole MAVLink change: drop the include line and delete
`hdgalt_dev.xml`.
...
1. Create a leaf MAVLink dialect file
   `modules/mavlink/message_definitions/v1.0/hdgalt_dev.xml` and pull
   it into the ArduPilot dialect with a single `<include>hdgalt_dev.xml</include>`
   line in `ardupilotmega.xml`. Inspect `ardupilotmega.xml` to see how
   other ArduPilot-specific messages (e.g. `RANGEFINDER`, `AHRS3`,
   `MEMINFO`) are declared.
2. Add `HDGALT_COMMAND` and `HDGALT_STATE` message definitions per
   section 5 of the design doc to `hdgalt_dev.xml`. Use currently-unused
   message IDs in the experimental 52100-52199 range. Document each
   field with a one-line `<description>` matching the design doc.
...
(which now includes `hdgalt_dev.xml`; the dialect is a gitignored
build artifact; upstream this rides with the mavlink-submodule PR +
standard pymavlink install). Locally: regenerate and run with
`PYTHONPATH=modules/mavlink`.
...
## Forks and upstreaming (the mavlink-submodule workflow)

HDGALT adds two MAVLink messages in `hdgalt_dev.xml` (a leaf dialect
included by `ardupilotmega.xml`), which live in the `modules/mavlink`
**submodule** (canonical: `github.com/ArduPilot/mavlink`). Submodule changes can't be pushed
to ArduPilot directly (no access), so development uses a personal
mavlink fork. This is the part that surprises people coming from
single-repo workflows; record for future reference.

### Repos involved

- `github.com/lvdlvd/ardupilot` — fork of ArduPilot/ardupilot. Work
  on branch `hdgaltmode`.
- `github.com/lvdlvd/mavlink` — fork of **ArduPilot/mavlink** (not
  mavlink/mavlink). Holds the `hdgalt_dev.xml` dialect and the
  one-line `ardupilotmega.xml` include on branch `hdgaltmode`.
- `pymavlink` is a submodule *of* mavlink. Its dialects
  (`dialects/v20/ardupilotmega.py`) are **gitignored, generated**
  from the XML — never forked or committed. They are regenerated by
  the standard pymavlink install (upstream/CI) or locally with
  `PYTHONPATH=modules/mavlink` after running `mavgen` on the XML.

Git tracks a submodule by **commit SHA**, not branch name. The
parent repo records "modules/mavlink is at SHA X"; it does not
"follow" the submodule's branch. After advancing the submodule you
must explicitly `git add modules/mavlink` in the parent and commit
the new pointer.

### Step A — fork-based development (DONE)

So the `hdgaltmode` branch is self-contained and CI/clone-able
before anything is upstreamed:

1. Fork ArduPilot/mavlink → `lvdlvd/mavlink`.
2. Push the submodule branch:
   `git -C modules/mavlink push <lvdlvd/mavlink remote> hdgaltmode`
   (commit `2f5675fa`).
3. Point the parent at the fork **on the `hdgaltmode` branch only**:
   `git config -f .gitmodules submodule.modules/mavlink.url \
   https://github.com/lvdlvd/mavlink` then `git submodule sync`,
   and commit `.gitmodules` (commit `c7b96cdb01`,
   "point mavlink to fork … while developing").
4. Bump + commit the parent's submodule pointer (`git add
   modules/mavlink`).

Result: `git clone --recurse-submodules` of
`lvdlvd/ardupilot@hdgaltmode` pulls mavlink from the fork and lands
the HDGALT XML automatically.

Day-to-day on the forks:

- Editing messages: `cd modules/mavlink`, edit
  `message_definitions/v1.0/hdgalt_dev.xml`, commit **inside the
  submodule**, push to `lvdlvd/mavlink`, then back in the parent
  `git add modules/mavlink` and commit the moved pointer. Two
  commits in two repos for one logical change — expected.
- After pulling someone else's parent commit that moves the
  pointer: run `git submodule update --init` or the build will use
  a stale mavlink.
- Regenerate pymavlink after an XML change before running the
  autotest (`mavgen` → `dialects/v20/ardupilotmega.py`); it is not
  committed.
- Build (`./waf`) regenerates the **C** headers from the XML
  automatically; only the **Python** dialect needs the manual step.

### Step B — upstreaming (FUTURE, when ready to submit)

Two PRs, in order, because the message contract must exist upstream
before ArduPlane can point at it:

1. **mavlink PR:** `lvdlvd/mavlink:hdgaltmode` →
   `ArduPilot/mavlink:master`, containing the `hdgalt_dev.xml` dialect
   plus the one-line `ardupilotmega.xml` include (messages, enums,
   `PLANE_MODE_HDGALT`). Reviewers may ask to fold these straight into
   `ardupilotmega.xml` and renumber out of the experimental range when
   upstreaming; that is a deliberate, single-file change to unwind.
   Get it reviewed/merged there.
2. **ardupilot PR:** on a branch off `hdgaltmode`:
   - **Revert/drop `c7b96cdb01`** so `.gitmodules` points back at
     `https://github.com/ArduPilot/mavlink` (`git submodule sync`).
     The fork URL must never reach upstream.
   - Bump `modules/mavlink` to the **merged ArduPilot/mavlink
     commit** from PR 1 (no URL change needed — the commit now
     lives in the canonical repo).
   - The ArduPlane code + autotest + design doc ride in this PR.
     Reference the design doc and the
     mavlink PR.

Until PR 1 merges, the ardupilot PR can't be opened cleanly (its
submodule pointer would reference a non-upstream commit). That's
why step A keeps the fork wiring on a private branch and step B
unwinds it.
